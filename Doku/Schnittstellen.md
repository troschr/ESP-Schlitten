...
| SET_SERVO | `CMD;<id>;SET_SERVO;position=<OPEN\|CLOSED\|SERVICE>` | nicht `ERROR`, nicht busy |
...
Der Schlitten trägt einen zusätzlichen Arm zum Öffnen der Druckertüren. Der Pi gibt nur die gewünschte logische Position vor – ob der Arm intern per Stepper oder Servo bewegt wird, ist noch nicht entschieden und für das Protokoll irrelevant.
...
| `SERVO_OPEN` | Servo auf OPEN gesetzt |
| `SERVO_CLOSED` | Servo auf CLOSED gesetzt |
| `SERVO_SERVICE` | Servo auf SERVICE gesetzt |
...
| Servo-Steuerung | Direkt per SET_SERVO vom Pi, oder implizit durch ESP-interne Sequenz? |
